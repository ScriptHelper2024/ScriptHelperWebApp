...
class CharacterProfileModel(EncryptedDocument):
    """
    CharacterProfileModel represents a character text document in MongoDB.

    Attributes:
        project_id (ReferenceField): A reference to the associated project document.
        character_key (UUIDField): A unique identifier for the character, shared across all versions.
        name (StringField): The name of the character
        character_order (IntField): The order of the character within the project.
        ...
        text_seed (StringField): The seed text used to generate or mutate the character text.
    ...
    meta = {
        ...
        'ordering': ['character_order', '-created_at'], # Ordered by 'character_order' and 'created_at' in descending order by default.
    ...
    character_key = UUIDField(binary=False, default=uuid.uuid4, unique=False)
    name = StringField()
    character_order = IntField()
    version_type = StringField(required=True, choices=('base', 'edit', 'note', 'magic-note', 'new', 'seed', 'generation'))
    source_version = ReferenceField('self')  # Allows reference to another CharacterProfileModel document.
    ...
    def _to_dict(self):
        ...
        return {
            ...
            'character_key': str(self.character_key),
            'name': self.name,
